...
with tabs[2]:
    ...
    st.markdown(
        r"""
    SVD of centered data \(X_c\) is \(X_c = U\Sigma V^\top\).
    
    The **principal directions** are the columns of \(V\), and the variance captured by each component is proportional to \(\sigma_i^2\) (scaled by \(n-1\)).
    
    If you “turn off” a \(\sigma_i\), you remove that component and the reconstruction degrades.
        """.strip()
    )
